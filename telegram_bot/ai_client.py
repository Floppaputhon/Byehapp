...
_GROUP_LIST_PATTERNS = _re.compile(
    r"(?:(?:в\s+)?(?:каких|какие|список)\s+(?:\w+\s+)?групп|"
    r"мои\s+групп|группы\s+(?:бота|мои|список|покажи)|"
    r"(?:покажи|показать|назови|перечисли|скажи)\s+(?:\w+\s+)*групп|"
    r"где\s+(?:ты\s+)?(?:есть|состоишь|добавлен)|"
    r"(?:в\s+каких|в\s+скольких)\s+(?:\w+\s+)?(?:группах|чатах))",
    _re.IGNORECASE,
)

_MODERATION_REMOTE_PATTERNS = _re.compile(
    r"(?:(?:замуть|замути|мут(?:ни)?|заткни|забань|забани|бань|банни|бан\b|кикни|кикнуть|кик\b|варни|предупреди|размуть|размути|разбань)\s+@\w|"
    r"(?:замуть|замути|мут(?:ни)?|заткни|забань|забани|бань|банни|бан\b|кикни|кикнуть|кик\b|варни|предупреди|размуть|размути|разбань)\s+(?:\S+\s+)*?(?:в\s+групп|на\s+\d|за\s+\S))",
    _re.IGNORECASE,
)
